var EditVehicle = function () {  
	...
	var resetParentVehicleOptions = function(exludeEntity){
    ...
    var handleValidation = function() {
        ...
        $.validator.addMethod("isProjectCode", function(value, element) {       
        	var vehicleCode = /^P\d{4}-?\d{3}-?\d{3}$/;  
        	return this.optional(element) || (vehicleCode.test(value));       
        }, "Please input correct vehicle code"); 
        ...
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                var vehicle = {
                		order: $("#form_edit #order").val(),
                		type: $("#form_edit #type").val(),
                		engine: $("#form_edit #engine").val(),
                		tire: $("#form_edit #tire").val(),
                		vin: $("#form_edit #vin").val(),
                		iLevel: $("#form_edit #iLevel").val()
                };
                update(id, vehicle);
    ...
    var inject = function(entity) {
    	//重置父产品选择框选项，不包含当前被编辑产品
    	resetParentVehicleOptions(entity);
    	...
    	if (result.success) {
            //给table赋值
            var vehicleInfo = result.data;
            $("#form_edit #id").val(vehicleInfo.id||"");
            $("#form_edit #order").val(vehicleInfo.order||"");
            $("#form_edit #type").val(vehicleInfo.type||"");
            $("#form_edit #engine").val(vehicleInfo.engine||"");
            $("#form_edit #tire").val(vehicleInfo.tire||"");
            $("#form_edit #vin").val(vehicleInfo.vin||"");
            $("#form_edit #iLevel").val(vehicleInfo.iLevel||"");
    ...
    //更新数据
    var update = function(id, vehicle) {	
    	var result = VehicleAjax.updateVehicle(id, vehicle);
        	if(!result.success){
